...
shinyServer(function(input, output) {
  ...
  output$text0 <- renderUI({
    input$goButton
    tmp <- isolate(totalEnc(DT = pop(), x0 = input$x0, iter = input$iter, p_Move = input$p_Move))
    text0 <- isolate(paste0("For a conference taking place in a square area of ", input$size,  " sq ft., where ", input$N, 
                   " people are present and out of which ", input$n, " are active participants, if we assume a beacon 
                   range of ", input$x0, " feet and we assume that ", 100*input$p_Move, "% of the total population 
